...
역으로 못 보냈다 했는데 보내지면 안 됨. 요약: 보내면 보낸 거고 못 보내면 못 보낸 거야 함.
...
a fillback 완료 후 INCR, 

b fillback 완료 후 INCR -> a랑 b가 같은 seq를 가지고 있음.
...
1이 제일 빨라서 최근 메시지가 1로 등록됨.

다음 2, 3이 1을 동시에 읽고, 나보다 작으니까 동시에 업데이트하려고 함.

근데 2가 더 느려서 최근 메시지가 3이 아니라 2로 등록됨.
